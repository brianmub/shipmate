# Design Presets & UI Theme Selection Rule

## Mandatory Workflow for UI & Frontend Design
Whenever starting a new project, feature, web app, landing page, dashboard, or UI design:
1. **Always ask the user which theme/preset they want** before writing CSS, HTML, or component styling.
2. Present the 8 design presets defined below:
   - **Preset 1: Editorial** (Fraunces + Source Sans 3, `#F7F3EC`, `#B23A2E`)
   - **Preset 2: Brutalist Mono** (Space Mono + Work Sans, `#FFFFFF`, `#000000`, `#FFE600`)
   - **Preset 3: Soft Organic** (DM Serif Display + Nunito Sans, `#FAF6F0`, `#7A8F6F`, `#C97B5A`)
   - **Preset 4: Corporate Trust** (IBM Plex Sans + IBM Plex Mono, `#F5F7FA`, `#0B2545`, `#1F7A8C`)
   - **Preset 5: Dark Technical** (Geist + Geist Mono, `#0A0B0D`, `#E6E8EB`, `#3DDC97`)
   - **Preset 6: Playful Bold** (Bricolage Grotesque + DM Sans, `#FFF4E0`, `#1A1A2E`, `#FF5D8F`)
   - **Preset 7: Luxury Minimal** (Cormorant Garamond + Jost, `#FBF9F5`, `#14120F`, `#A88B4A`)
   - **Preset 8: Swiss Grid** (Archivo, `#FFFFFF`, `#111111`, `#E10600`)
   - **Custom / Client Override**
3. Once a preset is selected by the user, strictly enforce the chosen preset's Global Rules:
   - Follow the chosen preset exactly: fonts, hex colors, radius, spacing. Do not substitute defaults.
   - Never use purple/indigo gradients, glassmorphism, or emoji as icons unless the preset specifies.
   - Do not use generic fonts (Inter, Roboto, Arial) unless the preset names them.
   - Avoid generic layouts (hero + 3 feature cards + testimonial row + CTA).
   - Define colors, fonts, radius, and spacing as CSS variables first, then build components from them.
   - Load fonts from Google Fonts with fallback stacks.
   - Use one accent color; everything else neutral.

---

# Design Presets Catalog
